...
function Privacy() {
  return <main className="legal-page">
    ...
    <section><h2>Information we collect</h2><h3>Account and profile information</h3><p>When you create an account, we collect your email address, password in hashed form, display name, username, profile picture if you add one, discoverability setting, friend-request setting, and account creation and update dates. If you continue with Google, Google provides an identity identifier, verified email address, and may provide your name. We do not receive your Google password.</p><h3>Content and social activity</h3><p>We store the wishlists, item titles, links, prices, quantities, images, notes, cash-fund details, recurring occasions, sharing choices, collaborators, friendships, groups, blocks, pins, gift claims, recipient notes, private gift-planning discussion comments, feedback you submit, and activity notifications you create or receive. Guest visitors to a shared list receive a random viewer token so their claims, notes, and discussion comments remain associated with that browser.</p><h3>Purchases</h3><p>Apple processes Hushful Pro purchases. We receive purchase status, product and transaction identifiers, and an app-account identifier needed to unlock or restore Pro. We do not receive your full payment-card details.</p><h3>Device, notification, and usage information</h3><p>If you enable push notifications, we store a device push token, platform, and app environment. On the web, we create a random browser identifier and record page paths, whether the visitor was signed in, and timestamps to understand aggregate use. Hushful does not use this information for cross-app tracking or targeted advertising. Local preferences, access tokens, guest-viewer tokens, and reminder information may be stored on your device or browser.</p><h3>Information collected automatically by infrastructure</h3><p>Our hosting, content-delivery, security, and email providers may process ordinary network information such as IP address, request time, user agent, and diagnostic logs to deliver and protect the Service. We do not sell this information.</p></section>
