...
export default function App() {
  ...
  useEffect(() => {
    if (!token) return
    setLoading(true)
    api.me(token).then((loadedUser) => { setUser(loadedUser); setOnboardingActive(!loadedUser.username || !loadedUser.privacySetupCompleted || loadedUser.onboardingVersion !== 1 || loadedUser.birthdaySetupCompleted === false) }).catch(onError).finally(() => setLoading(false))
  }, [token, onError])
  useEffect(() => {
    if (!toast) return
...
function OnboardingFlow({ token, user, userChanged, finish, logout }: { token: string; user: CurrentUser; userChanged: (user: CurrentUser) => void; finish: () => void | Promise<void>; logout: () => void }) {
  ...
  const [step, setStep] = useState<OnboardingStep>(() => coreSetupComplete && user.birthdaySetupCompleted === false ? 'birthday' : user.privacySetupCompleted && !user.username ? 'username' : 'tour')
  useEffect(() => {
  ...
  if (step === 'birthday') return <OnboardingBirthday token={token} completed={() => { userChanged({ ...user, birthdaySetupCompleted: true }); if (coreSetupComplete) void finish(); else setStep('first-list') }} logout={logout} />
...
function OnboardingBirthday({ token, completed, logout }: { token: string; completed: () => void; logout: () => void }) {
  const [birthday, setBirthday] = useState('2028-01-01')
  ...
  return <main className="auth-page onboarding-page"><section className="auth-card onboarding-card"><Logo /><p className="eyebrow">A thoughtful extra</p><h1>Want birthday reminders?</h1><p className="muted">Add your birthday so friends can plan ahead. Only the month and day are stored, and you can choose who can see it.</p><form className="stack-form" onSubmit={(event) => { event.preventDefault(); const [, month, day] = birthday.split('-').map(Number); if (month && day) void save({ birthdayMonth: month, birthdayDay: day, birthdayVisibility: visibility, birthdaySetupCompleted: true }) }}><Field label="Your birthday"><input type="date" value={birthday} onChange={(event) => setBirthday(event.target.value)} required /></Field><Field label="Who can see it"><select value={visibility} onChange={(event) => setVisibility(event.target.value as typeof visibility)}><option value="public">Everyone who can view my profile</option><option value="friends">Friends only</option><option value="private">Only me</option></select></Field>{error && <p className="auth-error" role="alert">{error}</p>}<button className="primary wide" disabled={busy}>{busy && <LoaderCircle className="spin" />} Save birthday <ChevronRight /></button><button type="button" className="text-button auth-switch" disabled={busy} onClick={() => void save({ birthdaySetupCompleted: true })}>Not now—I’ll add it later</button><button type="button" className="text-button auth-switch" onClick={logout}>Log out</button></form></section></main>
}
...
function GuestShareScreen({ shareToken }: { shareToken: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMentionCandidates(await api.mentionCandidates(shareToken, opened.viewerToken))
...
function Dashboard({ token, user, setUser, logout, onError, notify }: { token: string; user: CurrentUser; setUser: (u: CurrentUser) => void; logout: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <div className="app-shell">
    ...
    {accountOpen && <AccountModal token={token} user={user} userChanged={setUser} focusFeedback={feedbackFocus} close={() => { setAccountOpen(false); setFeedbackFocus(false) }} onError={onError} notify={notify} logout={logout} />}
...
function DiscussionPanel({ token, wishlistID, shareToken = '', viewerToken, accountShareID, mentionCandidates: suppliedCandidates, defaultName, onError }: { token?: string; wishlistID?: string; shareToken?: string; viewerToken?: string; accountShareID?: string; mentionCandidates?: SocialUser[]; defaultName: string; onError: (error: unknown) => void }) {
...
function OwnerItemCard({ item, planningRow, edit, remove }: { item: WishlistItem; planningRow?: SharedItemRow; edit: () => void; remove: () => void }) {
...
function SocialShareModal({ token, wishlist, close, notify, onError }: { token: string; wishlist: Wishlist; close: () => void; notify: (s: string) => void; onError: (e: unknown) => void }) {
  const [friends, setFriends] = useState<Friendship[]>([]), [groups, setGroups] = useState<FriendGroup[]>([]), [audience, setAudience] = useState<WishlistAudience>({ userIDs: [], groupIDs: [] }), [visibility, setVisibility] = useState<'private' | 'public'>('private'), [loading, setLoading] = useState(true), [saving, setSaving] = useState(false), [ready, setReady] = useState(false), [shareUrl, setShareUrl] = useState('')
  useEffect(() => { Promise.all([api.friends(token), api.friendGroups(token), api.wishlistAudience(token, wishlist.id), api.wishlistSettings(token, wishlist.id)]).then(([f, g, a, s]) => { setFriends(f); setGroups(g); setAudience(a); setVisibility(s.visibility); setReady(true) }).catch(onError).finally(() => setLoading(false)) }, [token, wishlist.id, onError])
  async function persist(nextAudience: WishlistAudience, nextVisibility: 'private' | 'public') { setSaving(true); try { await Promise.all([api.updateWishlistAudience(token, wishlist.id, nextAudience), api.updateWishlistSettings(token, wishlist.id, { visibility: nextVisibility })]); notify('Sharing updated') } catch (e) { onError(e) } finally { setSaving(false) } }
  ...
  return <Modal close={close} size="modal-wide"><ModalHeader eyebrow="Share without a link" title={`Share “${wishlist.title}”`} close={close} />{loading ? <FullPageLoader embedded /> : <div className="social-stack"><Field label="Wishlist visibility"><select value={visibility} onChange={(e) => { const next = e.target.value as 'private' | 'public'; setVisibility(next); if (ready) void persist(audience, next) }}><option value="private">Private</option><option value="public">Public</option></select></Field><p className="muted">{visibility === 'public' ? 'Anyone can view this list, and friends will see it on your profile. Private selections below remain available too.' : 'This list is hidden from the public. Only the friends and groups you select below can open it.'}</p>
...
function AccountModal({ token, user, userChanged, focusFeedback = false, close, onError, notify, logout }: { token: string; user: CurrentUser; userChanged: (user: CurrentUser) => void; focusFeedback?: boolean; close: () => void; onError: (e: unknown) => void; notify: (s: string) => void; logout: () => void }) {
  ...
  const [newAttributeLabel, setNewAttributeLabel] = useState(''), [newAttributeValue, setNewAttributeValue] = useState(''), [newAttributeVisibility, setNewAttributeVisibility] = useState<'public' | 'friends' | 'private'>('friends')
  ...
  useEffect(() => { api.metricsSummary(token).then(setMetrics).catch(() => undefined); api.adminFeedback(token).then(setFeedback).catch(() => undefined); api.adminReports(token).then(setReports).catch(() => undefined) }, [token])
  ...
  return <Modal close={close}><ModalHeader eyebrow="Your Hushful account" title="Account & privacy" close={close} /><form className="stack-form" onSubmit={(e) => e.preventDefault()}>
    ...
    {details && <section className="profile-details-panel"><div><strong>Birthday</strong><small>Only month and day are stored.</small></div><label className="checkbox"><input type="checkbox" checked={details.birthdayMonth != null && details.birthdayDay != null} onChange={(event) => { if (event.target.checked) { void saveDetails({ birthdayMonth: details.birthdayMonth || 1, birthdayDay: details.birthdayDay || 1, birthdayVisibility: details.birthdayVisibility }); } else { void saveDetails({ clearBirthday: true }) } }} /><span>Show my birthday on my profile</span></label>{details.birthdayMonth != null && details.birthdayDay != null && <><input type="date" value={`2028-${String(details.birthdayMonth).padStart(2, '0')}-${String(details.birthdayDay).padStart(2, '0')}`} onChange={(event) => { const [, month, day] = event.target.value.split('-').map(Number); if (month && day) void saveDetails({ birthdayMonth: month, birthdayDay: day, birthdayVisibility: details.birthdayVisibility }) }} /><select value={details.birthdayVisibility} onChange={(event) => void saveDetails({ birthdayMonth: details.birthdayMonth, birthdayDay: details.birthdayDay, birthdayVisibility: event.target.value as typeof details.birthdayVisibility })}><option value="public">Everyone who can view my profile</option><option value="friends">Friends only</option><option value="private">Only me</option></select></>}<hr /><div><strong>Custom profile details</strong><small>Share sizes, preferences, or any other helpful trait.</small></div>{details.attributes.map((attribute) => <article className="profile-attribute-editor" key={attribute.id}><input value={attribute.label} onChange={(event) => setDetails({ ...details, attributes: details.attributes.map((item) => item.id === attribute.id ? { ...item, label: event.target.value } : item) })} onBlur={() => void updateAttribute(attribute, { label: attribute.label })} /><input value={attribute.value} onChange={(event) => setDetails({ ...details, attributes: details.attributes.map((item) => item.id === attribute.id ? { ...item, value: event.target.value } : item) })} onBlur={() => void updateAttribute(attribute, { value: attribute.value })} /><select value={attribute.visibility} onChange={(event) => void updateAttribute(attribute, { visibility: event.target.value as typeof attribute.visibility })}><option value="public">Public</option><option value="friends">Friends</option><option value="private">Only me</option></select><button type="button" className="text-button danger-text" onClick={() => void deleteAttribute(attribute.id)}>Remove</button></article>)}<div className="profile-attribute-editor"><input placeholder="Attribute (e.g. Shoe size)" value={newAttributeLabel} onChange={(event) => setNewAttributeLabel(event.target.value)} /><input placeholder="Value (e.g. 10.5)" value={newAttributeValue} onChange={(event) => setNewAttributeValue(event.target.value)} /><select value={newAttributeVisibility} onChange={(event) => setNewAttributeVisibility(event.target.value as typeof newAttributeVisibility)}><option value="public">Public</option><option value="friends">Friends</option><option value="private">Only me</option></select><button type="button" className="secondary" disabled={!newAttributeLabel.trim() || !newAttributeValue.trim()} onClick={() => void addAttribute()}>Add</button></div></section>}
    ...
    {accounts && <section className="feedback-admin"><div><strong>Account directory</strong><small>{accounts.length} account{accounts.length === 1 ? '' : 's'} · newest first</small></div><input value={accountSearch} onChange={(event) => setAccountSearch(event.target.value)} placeholder="Search name, username, or email" />{accounts.length === 0 ? <p className="hint">No accounts created yet.</p> : <div className="feedback-responses">{accounts.map((account) => <article key={account.id}><div><strong>{account.displayName || 'No display name'}</strong><small>{account.createdAt ? new Date(account.createdAt).toLocaleString() : 'Unknown signup date'} · {account.emailVerified === false ? 'unverified' : 'verified'}{account.isPro ? ' · Pro' : ''}{account.suspicious ? ' · review' : ''}</small></div><small>{account.email}{account.username ? ` · @${account.username}` : ''}</small></article>)}</div>}</section>}
