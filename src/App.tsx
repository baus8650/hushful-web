...
function Dashboard({ token, user, setUser, logout, onError, notify }: { token: string; user: CurrentUser; setUser: (u: CurrentUser) => void; logout: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
      <div className="sidebar-top"><Logo compact onClick={() => select({ kind: 'home' })} /><button className="icon-button mobile-close" onClick={() => setMobileNav(false)}><X /></button></div>
      <nav>
    ...
    <main className="main-panel">
      <header className="mobile-header"><button className="icon-button" onClick={() => setMobileNav(true)}><Menu /></button><Logo compact onClick={() => select({ kind: 'home' })} /><span /></header>
...
function purchaseEvidenceLabel(status?: string) {
  switch (status) {
    case 'verified_current_account': return 'Verified purchase · this Hushful account'
    case 'verified_current_account_inactive': return 'Purchase found · no longer active'
    case 'verified_other_account': return 'Verified purchase · another Hushful account'
    case 'verified_unbound_purchase': return 'Verified purchase · not linked to a Hushful account'
    default: return 'No verified purchase found'
  }
}

function purchaseEvidenceIsVerified(status?: string) {
  return status === 'verified_current_account' || status === 'verified_current_account_inactive' || status === 'verified_other_account' || status === 'verified_unbound_purchase'
}

function purchaseEvidenceCanGrant(status?: string) {
  // A mismatched or unbound token proves that a purchase exists, but not that
  // the feedback submitter is entitled to receive it. Keep manual recovery
  // grants off those cards unless an administrator verifies identity through a
  // separate process.
  return status === 'verified_current_account'
}

function FeedbackThreadPanel({ thread, reply, setReply, busy, send, close, admin = false }: { thread: Awaited<ReturnType<typeof api.feedbackThread>>; reply: string; setReply: (value: string) => void; busy: boolean; send: () => void; close: () => void; admin?: boolean }) {
  return <section className="feedback-admin feedback-thread"><div><strong>{admin ? 'Feedback conversation' : 'Your feedback conversation'}</strong><button type="button" className="text-button" onClick={close}>Close</button></div><div className="feedback-thread-messages"><article><div><span>{thread.feedback.category}</span><small>{thread.feedback.createdAt ? new Date(thread.feedback.createdAt).toLocaleString() : 'Submitted'}</small></div><p>{thread.feedback.message}</p></article>{thread.replies.map((item) => <article key={item.id} className={item.authorRole === 'admin' ? 'support-reply' : ''}><div><span>{item.authorRole === 'admin' ? 'Hushful Support' : 'You'}</span><small>{item.createdAt ? new Date(item.createdAt).toLocaleString() : ''}</small></div><p>{item.message}</p></article>)}</div><Field label="Reply"><textarea rows={3} maxLength={4000} value={reply} onChange={(event) => setReply(event.target.value)} placeholder={admin ? 'Reply to the user…' : 'Add information or reply to support…'} /></Field><button type="button" className="secondary" disabled={busy || !reply.trim()} onClick={send}>{busy ? 'Sending…' : 'Send reply'}</button></section>
}

function AccountActivityModal({ account, activity, close }: { account: AdminAccount; activity: AdminAccountActivity; close: () => void }) {
  const date = (value?: string) => value ? new Date(value).toLocaleString() : 'Unknown date'
  const requests = (title: string, entries: AdminAccountActivity['sentFriendRequests']) => <section className="feedback-admin"><div><strong>{title}</strong><small>{entries.length} current request{entries.length === 1 ? '' : 's'}</small></div>{entries.length === 0 ? <p className="hint">None.</p> : <div className="feedback-responses">{entries.map((request) => <article key={request.id}><div><strong>{request.otherUserDisplayName || request.otherUserEmail}</strong><small>{request.status} · {date(request.createdAt)}</small></div><small>{request.otherUserEmail}{request.otherUsername ? ` · @${request.otherUsername}` : ''}</small></article>)}</div>}</section>
  return <Modal close={close} size="modal-wide"><ModalHeader eyebrow="Admin review" title="Account activity" close={close} /><div className="stack-form"><section className="feedback-admin"><div><strong>{account.displayName || activity.email}</strong><small>{activity.email}</small></div><p className="hint">{activity.emailVerified ? 'Verified account.' : 'Unverified accounts cannot sign in or create activity.'}</p></section><section className="feedback-admin"><div><strong>Lists created</strong><small>{activity.createdWishlists.length} current list{activity.createdWishlists.length === 1 ? '' : 's'}</small></div>{activity.createdWishlists.length === 0 ? <p className="hint">No lists created.</p> : <div className="feedback-responses">{activity.createdWishlists.map((list) => <article key={list.id}><div><strong>{list.title}</strong><small>{list.archived ? 'Archived' : 'Active'} · {date(list.createdAt)}</small></div></article>)}</div>}</section>{requests('Friend requests sent', activity.sentFriendRequests)}{requests('Friend requests received', activity.receivedFriendRequests)}<p className="hint">This shows current records only. Deleted lists and removed or declined friend requests cannot be reconstructed.</p></div><div className="modal-actions"><button type="button" className="secondary" onClick={close}>Done</button></div></Modal>
}

type ProGrantTarget = { id: string; email?: string; displayName?: string | null }
...
function AccountModal({ token, user, userChanged, onSharedListsPreferenceChanged, focusFeedback = false, close, onError, notify, logout }: { token: string; user: CurrentUser; userChanged: (user: CurrentUser) => void; onSharedListsPreferenceChanged?: () => void; focusFeedback?: boolean; close: () => void; onError: (e: unknown) => void; notify: (s: string) => void; logout: () => void }) {
  ...
  const [friends, setFriends] = useState<Friendship[]>([])
  const [blockedUsers, setBlockedUsers] = useState<SocialUser[] | null>(null)
  const [unblockingUserID, setUnblockingUserID] = useState<string | null>(null)
  const [audienceAttribute, setAudienceAttribute] = useState<ProfileAttribute | null>(null)
  const [newAttributeLabel, setNewAttributeLabel] = useState(''), [newAttributeValue, setNewAttributeValue] = useState(''), [newAttributeVisibility, setNewAttributeVisibility] = useState<'public' | 'friends' | 'selected' | 'private'>('private')
  ...
  const [myFeedback, setMyFeedback] = useState<Awaited<ReturnType<typeof api.feedback>> | null>(null)
  const [feedbackThread, setFeedbackThread] = useState<Awaited<ReturnType<typeof api.feedbackThread>> | null>(null)
  const [feedbackThreadIsAdmin, setFeedbackThreadIsAdmin] = useState(false)
  const [feedbackReplyMessage, setFeedbackReplyMessage] = useState('')
  const [feedbackThreadBusy, setFeedbackThreadBusy] = useState(false)
  ...
  const [accountActivity, setAccountActivity] = useState<{ account: AdminAccount; activity: AdminAccountActivity } | null>(null)
  const [accountActivityBusy, setAccountActivityBusy] = useState<string | null>(null)
  ...
  useEffect(() => { api.metricsSummary(token).then(setMetrics).catch(() => undefined); api.adminFeedback(token, showArchivedFeedback).then(setFeedback).catch(() => undefined); api.feedback(token).then(setMyFeedback).catch(() => undefined); api.adminReports(token).then(setReports).catch(() => undefined); api.adminAudit(token).then(setAudit).catch(() => undefined); api.adminProGrants(token).then(setProGrants).catch(() => undefined) }, [token, showArchivedFeedback])
  useEffect(() => { api.adminTOTPStatus(token).then(setAdminTotp).catch(() => undefined) }, [token])
  useEffect(() => { api.profileDetails(token).then(setDetails).catch(onError); api.friends(token).then((items) => setFriends(items.filter((item) => item.status === 'accepted'))).catch(() => undefined) }, [token, onError])
  useEffect(() => { api.blockedUsers(token).then(setBlockedUsers).catch((error) => { setBlockedUsers([]); onError(error) }) }, [token, onError])
  ...
  async function submitFeedback() { const message = feedbackMessage.trim(); if (!message) return; setFeedbackBusy(true); setFeedbackError(''); try { await api.submitFeedback(token, feedbackCategory, message); setFeedbackMessage(''); void api.feedback(token).then(setMyFeedback).catch(() => undefined); notify('Thank you—your feedback was received') } catch (e) { setFeedbackError(e instanceof Error ? e.message : 'Unable to submit feedback.') } finally { setFeedbackBusy(false) } }
  async function openFeedbackThread(id: string, admin = false) { setFeedbackThreadBusy(true); try { setFeedbackThread(admin ? await api.adminFeedbackThread(token, id) : await api.feedbackThread(token, id)); setFeedbackThreadIsAdmin(admin); setFeedbackReplyMessage('') } catch (error) { onError(error) } finally { setFeedbackThreadBusy(false) } }
  async function replyToFeedback() { const message = feedbackReplyMessage.trim(); const id = feedbackThread?.feedback.id; if (!message || !id) return; setFeedbackThreadBusy(true); try { setFeedbackThread(feedbackThreadIsAdmin ? await api.adminReplyToFeedback(token, id, message) : await api.replyToFeedback(token, id, message)); setFeedbackReplyMessage(''); notify('Reply sent'); if (feedbackThreadIsAdmin) setFeedback(await api.adminFeedback(token, showArchivedFeedback)); else setMyFeedback(await api.feedback(token)) } catch (error) { onError(error) } finally { setFeedbackThreadBusy(false) } }
  async function updateFeedback(id: string, action: 'close' | 'reopen' | 'archive' | 'unarchive') { setFeedbackThreadBusy(true); try { const updated = action === 'close' ? await api.closeFeedback(token, id) : action === 'reopen' ? await api.reopenFeedback(token, id) : action === 'archive' ? await api.archiveFeedback(token, id) : await api.unarchiveFeedback(token, id); setFeedback((current) => current?.map((entry) => entry.id === id ? updated : entry) || current); if (feedbackThread?.feedback.id === id) setFeedbackThread((current) => current ? { ...current, feedback: updated } : current); if (action === 'archive' && !showArchivedFeedback) setFeedback((current) => current?.filter((entry) => entry.id !== id) || current); notify(action === 'close' ? 'Feedback closed' : action === 'reopen' ? 'Feedback reopened' : action === 'archive' ? 'Feedback archived' : 'Feedback restored') } catch (error) { onError(error) } finally { setFeedbackThreadBusy(false) } }
  ...
  async function grantProAccess(target: ProGrantTarget) {
    const existing = proGrants?.find((grant) => grant.userID === target.id && grant.active)
    if (existing) { notify(`Manual Pro access is already active for ${target.email || target.displayName || 'this account'}`); return }
    const accountLabel = target.email || target.displayName || 'this account'
    const reason = window.prompt('Why should Pro access be granted for this account? Review Purchase feedback evidence first; an unverified claim is not proof.', 'Verified store purchase; manual recovery grant approved')?.trim()
    if (!reason) return
    setProGrantBusy(target.id)
    try {
      const grant = await api.grantProAccess(token, target.id, reason)
      setProGrants((current) => [...(current || []).filter((item) => item.userID !== target.id || !item.active), grant])
      setAccounts((current) => current ? current.map((item) => item.id === target.id ? { ...item, isPro: true } : item) : current)
      notify(`Pro access granted to ${accountLabel}`)
    } catch (e) { onError(e) } finally { setProGrantBusy(null) }
  }
  ...
  async function viewAccountActivity(account: AdminAccount) { setAccountActivityBusy(account.id); try { setAccountActivity({ account, activity: await api.adminAccountActivity(token, account.id) }) } catch (e) { onError(e) } finally { setAccountActivityBusy(null) } }
  ...
  async function unblock(userToUnblock: SocialUser) {
    const label = userToUnblock.displayName || `@${userToUnblock.username}`
    if (!window.confirm(`Unblock ${label}? They may be able to find or contact you again, depending on your privacy settings.`)) return
    setUnblockingUserID(userToUnblock.id)
    try {
      await api.unblockUser(token, userToUnblock.id)
      setBlockedUsers((current) => current?.filter((item) => item.id !== userToUnblock.id) || current)
      notify(`${label} unblocked`)
    } catch (error) { onError(error) } finally { setUnblockingUserID(null) }
  }
  return <Modal close={close} size="modal-profile"><ModalHeader eyebrow="Your Hushful account" title="Account & privacy" close={close} /><form className="stack-form" onSubmit={(e) => e.preventDefault()}>
    ...
    <section className="profile-details-panel"><div><strong>Blocked accounts</strong><small>People you block cannot find or contact you through Hushful. You can unblock them here at any time.</small></div>{blockedUsers === null ? <p className="hint">Loading blocked accounts…</p> : blockedUsers.length === 0 ? <p className="hint">You have not blocked anyone.</p> : <div className="social-stack">{blockedUsers.map((blockedUser) => <div className="social-row" key={blockedUser.id}><Avatar name={blockedUser.displayName || blockedUser.username} userId={blockedUser.id} hasAvatar={blockedUser.hasAvatar} /><span><strong>{blockedUser.displayName || `@${blockedUser.username}`}</strong><small>@{blockedUser.username}</small></span><button type="button" className="secondary" disabled={unblockingUserID === blockedUser.id} onClick={() => void unblock(blockedUser)}>{unblockingUserID === blockedUser.id ? 'Unblocking…' : 'Unblock'}</button></div>)}</div>}</section>
    ...
    {details && <section className="profile-details-panel"><div><strong>Birthday</strong><small>Your birthday stays private by default. Share only the month and day if you want people to set reminders.</small></div>{details.birthdayMonth != null && details.birthdayDay != null && details.birthdayYear != null && <><input type="date" value={`${details.birthdayYear}-${String(details.birthdayMonth).padStart(2, '0')}-${String(details.birthdayDay).padStart(2, '0')}`} onChange={(event) => { const [year, month, day] = event.target.value.split('-').map(Number); if (year && month && day) void saveDetails({ birthdayYear: year, birthdayMonth: month, birthdayDay: day, birthdayVisibility: details.birthdayVisibility }) }} /><select value={details.birthdayVisibility} onChange={(event) => void saveDetails({ birthdayYear: details.birthdayYear, birthdayMonth: details.birthdayMonth, birthdayDay: details.birthdayDay, birthdayVisibility: event.target.value as typeof details.birthdayVisibility })}><option value="private">Only me</option><option value="friends">Friends only</option><option value="public">Everyone who can view my profile</option></select></>}{user.ageBand === 'adult' && <label className="checkbox"><input type="checkbox" checked={details.matureProfileEnabled === true} onChange={(event) => void saveDetails({ matureProfileEnabled: event.target.checked })} /><span><strong>Limit this profile to adults</strong><small>Use this when the profile may include content intended for adults. People under 18 will not see this profile.</small></span></label>}<hr /><div><strong>Custom profile details</strong><small>Share sizes, preferences, or any other helpful trait.</small></div>{details.attributes.map((attribute) => <article className="profile-attribute-editor" key={attribute.id}><input value={attribute.label} onChange={(event) => setDetails({ ...details, attributes: details.attributes.map((item) => item.id === attribute.id ? { ...item, label: event.target.value } : item) })} onBlur={() => void updateAttribute(attribute, { label: attribute.label })} /><input value={attribute.value} onChange={(event) => setDetails({ ...details, attributes: details.attributes.map((item) => item.id === attribute.id ? { ...item, value: event.target.value } : item) })} onBlur={() => void updateAttribute(attribute, { value: attribute.value })} /><select value={attribute.visibility} onChange={(event) => void updateAttribute(attribute, { visibility: event.target.value as typeof details.attributes[number]['visibility'] })}><option value="public">Public</option><option value="friends">Friends only</option><option value="selected">Selected people</option><option value="private">Only me</option></select>{attribute.visibility === 'selected' && <button type="button" className="secondary" disabled={friends.length === 0} onClick={() => setAudienceAttribute(attribute)}>{friends.length === 0 ? 'Add accepted friends first' : `Choose people (${attribute.selectedUserIDs?.length ?? 0})`}</button>}<button type="button" className="text-button danger-text" onClick={() => void deleteAttribute(attribute.id)}>Remove</button></article>)}<div className="profile-attribute-editor"><input placeholder="Attribute (e.g. Shoe size)" value={newAttributeLabel} onChange={(event) => setNewAttributeLabel(event.target.value)} /><input placeholder="Value (e.g. 10.5)" value={newAttributeValue} onChange={(event) => setNewAttributeValue(event.target.value)} /><select value={newAttributeVisibility} onChange={(event) => setNewAttributeVisibility(event.target.value as typeof newAttributeVisibility)}><option value="public">Public</option><option value="friends">Friends only</option><option value="selected">Selected people</option><option value="private">Only me</option></select><button type="button" className="secondary" disabled={!newAttributeLabel.trim() || !newAttributeValue.trim()} onClick={() => void addAttribute()}>Add</button></div>{newAttributeVisibility === 'selected' && <small className="hint">After adding it, choose the accepted friends in the picker.</small>}</section>}
    <section ref={feedbackPanelRef} className="feedback-panel"><div><strong>Send feedback</strong><small>Ideas, problems, and little details all help make Hushful better.</small><small>I’m a team of one, and feedback and suggestions are always welcome. I want Hushful to be as useful and enjoyable as possible, so I’ll get back to you as soon as I can. Thanks for your patience.</small></div><Field label="Category"><select value={feedbackCategory} onChange={(e) => { const value = e.target.value; setFeedbackCategory(value); if (value === 'purchase') setFeedbackMessage('I am trying to restore Hushful Pro, but the purchase is linked to another Hushful account. Please verify this purchase and tell me what to do next.') }}><option value="general">General</option><option value="idea">Idea</option><option value="problem">Problem</option><option value="praise">Praise</option><option value="purchase">Purchase</option></select></Field>{feedbackCategory === 'purchase' ? <p className="hint">Purchase claims include your Hushful account email and cannot be anonymous. They are reviewed against store records; a claim by itself does not unlock Pro.</p> : null}<Field label="Your feedback"><textarea rows={4} maxLength={4000} value={feedbackMessage} onChange={(e) => setFeedbackMessage(e.target.value)} placeholder="Tell us what’s on your mind…" /></Field><small className="feedback-count">{feedbackMessage.length}/4,000</small>{feedbackError && <p className="form-error" role="alert">{feedbackError}</p>}<button type="button" className="secondary" disabled={feedbackBusy || !feedbackMessage.trim()} onClick={() => void submitFeedback()}>{feedbackBusy ? <LoaderCircle className="spin" /> : <Send />} Submit feedback</button></section>
    {myFeedback && <section className="feedback-admin"><div><strong>Submitted feedback</strong><small>Follow replies from Hushful support</small></div>{myFeedback.length === 0 ? <p className="hint">You have not submitted feedback yet.</p> : <div className="feedback-responses">{myFeedback.map((entry) => <article key={entry.id}><div><span>{entry.category}</span><small>{entry.status === 'closed' ? 'Closed' : 'Open'} · {entry.createdAt ? new Date(entry.createdAt).toLocaleString() : 'Just now'}</small></div><p>{entry.message}</p><button type="button" className="text-button" disabled={feedbackThreadBusy} onClick={() => void openFeedbackThread(entry.id)}>View conversation</button></article>)}</div>}</section>}
    {feedbackThread && !feedbackThreadIsAdmin && <FeedbackThreadPanel thread={feedbackThread} reply={feedbackReplyMessage} setReply={setFeedbackReplyMessage} busy={feedbackThreadBusy} send={() => void replyToFeedback()} close={() => setFeedbackThread(null)} />}
    ...
    {accounts && <section className="feedback-admin"><div><strong>Account directory</strong><small>{accounts.length} account{accounts.length === 1 ? '' : 's'} · newest first</small></div><p className="hint">If a user paid but Pro did not unlock, find the account below and grant a manual recovery entitlement only after reviewing verified evidence. Google Play does not expose the purchaser email for one-time products, so verify cross-account claims in Play Console using the order ID before granting. This does not transfer or alter the store purchase.</p><input value={accountSearch} onChange={(event) => setAccountSearch(event.target.value)} placeholder="Search name, username, or email" />{accounts.length === 0 ? <p className="hint">No accounts created yet.</p> : <div className="feedback-responses">{accounts.map((account) => { const manualGrant = proGrants?.find((grant) => grant.userID === account.id && grant.active); return <article key={account.id}><div><strong>{account.displayName || 'No display name'}</strong><small>{account.createdAt ? new Date(account.createdAt).toLocaleString() : 'Unknown signup date'} · {account.emailVerified === false ? 'unverified — cannot sign in' : 'verified'}{account.isPro ? ' · Pro active' : ''}</small></div><small>{account.email}{account.username ? ` · @${account.username}` : ''}</small>{account.suspicious && <p className="suspicious-account"><CircleAlert size={16} />Generated-looking account — review before taking action</p>}{manualGrant && <small>Manual grant: {manualGrant.reason}</small>}<div className="modal-actions spread"><button type="button" className="text-button" disabled={accountActivityBusy === account.id} onClick={() => void viewAccountActivity(account)}>{accountActivityBusy === account.id ? 'Loading…' : 'View activity'}</button>{manualGrant ? <button type="button" className="text-button danger-text" disabled={proGrantBusy === account.id} onClick={() => void revokeProAccess(manualGrant)}>{proGrantBusy === account.id ? 'Revoking…' : 'Revoke manual Pro grant'}</button> : account.emailVerified === false ? <small>Pro access is unavailable until email verification.</small> : <button type="button" className="secondary" disabled={proGrantBusy === account.id} onClick={() => void grantProAccess(account)}>{proGrantBusy === account.id ? 'Granting…' : 'Grant Pro access'}</button>}</div></article> })}</div>}</section>}
    {accountActivity && <AccountActivityModal account={accountActivity.account} activity={accountActivity.activity} close={() => setAccountActivity(null)} />}
    {feedback && <section className="feedback-admin">
      <div><strong>User feedback</strong><small>{feedback.length} active response{feedback.length === 1 ? '' : 's'}</small></div>
      <label className="checkbox"><input type="checkbox" checked={showArchivedFeedback} onChange={(event) => setShowArchivedFeedback(event.target.checked)} /><span><strong>Show archived</strong><small>Archived feedback stays out of this list by default.</small></span></label>
      {feedback.length === 0 ? <p className="hint">No feedback submitted yet.</p> : <div className="feedback-responses">{feedback.map((entry) => {
        const manualGrant = proGrants?.find((grant) => grant.userID === entry.userID && grant.active)
        return <article key={entry.id}>
          <div><span>{entry.category}</span><small>{entry.platform.toUpperCase()} · {entry.status === 'closed' ? 'Closed' : 'Open'}{entry.archived ? ' · Archived' : ''} · {entry.createdAt ? new Date(entry.createdAt).toLocaleString() : 'Just now'}</small></div>
          <p>{entry.message}</p>
          <div className="modal-actions"><button type="button" className="text-button" disabled={feedbackThreadBusy} onClick={() => void openFeedbackThread(entry.id, true)}>Conversation</button>{entry.status === 'closed' ? <button type="button" className="text-button" disabled={feedbackThreadBusy} onClick={() => void updateFeedback(entry.id, 'reopen')}>Reopen</button> : <button type="button" className="text-button" disabled={feedbackThreadBusy} onClick={() => void updateFeedback(entry.id, 'close')}>Close</button>}{entry.archived ? <button type="button" className="text-button" disabled={feedbackThreadBusy} onClick={() => void updateFeedback(entry.id, 'unarchive')}>Restore</button> : <button type="button" className="text-button" disabled={feedbackThreadBusy} onClick={() => void updateFeedback(entry.id, 'archive')}>Archive</button>}</div>
          {entry.category === 'purchase' && <div className={`purchase-review ${purchaseEvidenceIsVerified(entry.purchaseEvidence) ? 'verified' : 'unverified'}`}>
            <strong>{purchaseEvidenceLabel(entry.purchaseEvidence)}</strong>
            <small>{entry.purchaseEvidenceDetails || 'No store verification was available. Do not grant Pro from this claim alone.'}</small>
            {entry.purchaseEvidence === 'verified_other_account' && <small>This purchase belongs to another Hushful account. A manual grant here does not transfer the Google Play purchase.</small>}
            {entry.purchaseProvider === 'google_play' && <small>Google Play does not provide the purchaser email for this one-time product. Use the order ID in Play Console for any cross-account review.</small>}
            {entry.purchaseProvider && <small>Store: {entry.purchaseProvider === 'google_play' ? 'Google Play' : 'Apple'}{entry.purchaseAt ? ` · ${new Date(entry.purchaseAt).toLocaleString()}` : ''}{entry.purchaseOrderID ? ` · Order ${entry.purchaseOrderID}` : ''}</small>}
            {purchaseEvidenceCanGrant(entry.purchaseEvidence) && (manualGrant
              ? <small className="purchase-grant-status">Manual Pro access is already active for this account.</small>
              : <button type="button" className="secondary" disabled={proGrantBusy === entry.userID} onClick={() => void grantProAccess({ id: entry.userID, email: entry.userEmail, displayName: entry.userDisplayName })}>{proGrantBusy === entry.userID ? 'Granting…' : 'Grant Pro to this account'}</button>)}
          </div>}
          <small>{entry.category === 'purchase' ? `Submitted by ${entry.userEmail || 'unknown account email'}` : `${entry.userDisplayName ? `${entry.userDisplayName} · ` : ''}${entry.userEmail || 'Anonymous'}`}</small>
        </article>
      })}</div>}
    </section>}
    {feedbackThread && feedbackThreadIsAdmin && <FeedbackThreadPanel thread={feedbackThread} reply={feedbackReplyMessage} setReply={setFeedbackReplyMessage} busy={feedbackThreadBusy} send={() => void replyToFeedback()} close={() => setFeedbackThread(null)} admin />}
  ...
  </form>{audienceAttribute && <ProfileAttributeAudienceModal attribute={audienceAttribute} friends={friends} close={() => setAudienceAttribute(null)} save={(selectedUserIDs) => { const updated = { ...audienceAttribute, selectedUserIDs }; setDetails((current) => current ? { ...current, attributes: current.attributes.map((item) => item.id === updated.id ? updated : item) } : current); void updateAttribute(updated, { selectedUserIDs }); setAudienceAttribute(null) }} />}</Modal>
}

function ProfileAttributeAudienceModal({ attribute, friends, close, save }: { attribute: ProfileAttribute; friends: Friendship[]; close: () => void; save: (selectedUserIDs: string[]) => void }) {
  const [selectedUserIDs, setSelectedUserIDs] = useState<string[]>(attribute.selectedUserIDs ?? [])
  function toggle(userID: string) { setSelectedUserIDs((current) => current.includes(userID) ? current.filter((id) => id !== userID) : [...current, userID]) }
  return <Modal close={close} size="modal-profile"><ModalHeader eyebrow={attribute.label} title="Choose people" close={close} /><p className="muted modal-copy">Choose which accepted friends can see this detail.</p>{friends.length === 0 ? <p className="hint">Add accepted friends before selecting an audience.</p> : <div className="stack-form">{friends.map((friend) => <label className="checkbox" key={friend.user.id}><input type="checkbox" checked={selectedUserIDs.includes(friend.user.id)} onChange={() => toggle(friend.user.id)} /><span>{friend.user.displayName || `@${friend.user.username}`}</span></label>)}</div>}<div className="modal-actions"><button type="button" className="secondary" onClick={close}>Cancel</button><button type="button" className="primary" disabled={friends.length === 0} onClick={() => save(selectedUserIDs)}>Done</button></div></Modal>
}
...
function Logo({ compact = false, onClick }: { compact?: boolean; onClick?: () => void }) {
  const className = `logo ${compact ? 'compact' : ''}`
  const content = <><span><Sparkles /></span><strong>hushful</strong></>
  return onClick ? <button type="button" className={`${className} logo-button`} onClick={onClick} aria-label="Go to Overview">{content}</button> : <div className={className}>{content}</div>
}
