...
export default function App() {
  ...
  useEffect(() => {
    if (!token) return
    setLoading(true)
    api.me(token).then((loadedUser) => { setUser(loadedUser); setOnboardingActive(!loadedUser.username || !loadedUser.privacySetupCompleted || loadedUser.onboardingVersion !== 1 || loadedUser.ageBand === 'unknown' || loadedUser.birthdaySetupCompleted !== true) }).catch(onError).finally(() => setLoading(false))
  }, [token, onError])
  useEffect(() => {
    if (!toast) return
...
function OnboardingFlow({ token, user, userChanged, finish, logout }: { token: string; user: CurrentUser; userChanged: (user: CurrentUser) => void; finish: () => void | Promise<void>; logout: () => void }) {
  const coreSetupComplete = Boolean(user.username && user.privacySetupCompleted && user.onboardingVersion === 1 && user.ageBand !== 'unknown')
  const establishedAccount = Boolean(user.username && user.privacySetupCompleted && user.onboardingVersion === 1)
  const [step, setStep] = useState<OnboardingStep>(() => !user.username ? 'tour' : !user.privacySetupCompleted ? 'privacy' : (user.birthdaySetupCompleted !== true || user.ageBand === 'unknown') ? 'birthday' : 'tour')
  useEffect(() => {
...
function SharedItemCard({ row, accessToken, viewerToken, chooseQuantity, editNote, removeNote }: { row: SharedItemRow; accessToken?: string; viewerToken?: string; chooseQuantity: (quantity: number) => void; editNote: (note?: SharedItemRow['notes'][number]) => void; removeNote: () => void }) {
