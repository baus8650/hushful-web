...
export default function App() {
  ...
  useEffect(() => {
    if (!token) return
    setLoading(true)
    api.me(token).then((loadedUser) => { setUser(loadedUser); setOnboardingActive(!loadedUser.username || !loadedUser.privacySetupCompleted || loadedUser.onboardingVersion !== 1) }).catch(onError).finally(() => setLoading(false))
  }, [token, onError])
  useEffect(() => {
    if (!toast) return
