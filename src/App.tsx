...
function TutorialModal({ close }: { close: () => void }) { const [page, setPage] = useState(0); const steps = [{ icon: <Sparkles />, title: 'Gather what you love', text: 'Create a wishlist, then add each item’s link, price, quantity, and helpful details.' }, { icon: <Link2 />, title: 'Share it privately', text: 'Send your private Hushful link to friends and family. Only people with the link can open it.' }, { icon: <Gift />, title: 'Keep the surprise', text: 'Recipients coordinate quantities and notes together. Their claims stay hidden from the list owner.' }]; const step = steps[page]; return <Modal close={close}><div className="tutorial"><Logo compact /><div className="tutorial-art">{step.icon}</div><p className="eyebrow">Step {page + 1} of {steps.length}</p><h2>{step.title}</h2><p>{step.text}</p><div className="tutorial-dots">{steps.map((_, index) => <span key={index} className={index === page ? 'active' : ''} />)}</div><div className="modal-actions spread"><button className="text-button" onClick={close}>{page === steps.length - 1 ? 'Close' : 'Skip'}</button><button className="primary" onClick={() => page === steps.length - 1 ? close() : setPage(page + 1)}>{page === steps.length - 1 ? 'Start using Hushful' : 'Next'} <ChevronRight /></button></div></div></Modal> }
