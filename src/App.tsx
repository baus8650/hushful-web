...
function Dashboard({ token, user, setUser, logout, onError, notify }: { token: string; user: CurrentUser; setUser: (u: CurrentUser) => void; logout: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <div className="app-shell">
    ...
    {accountOpen && <AccountModal token={token} user={user} userChanged={setUser} focusFeedback={feedbackFocus} close={() => { setAccountOpen(false); setFeedbackFocus(false) }} save={async (profile) => { try { setUser(await api.updateProfile(token, profile)); setAccountOpen(false); notify('Profile updated') } catch (e) { onError(e) } }} onError={onError} notify={notify} logout={logout} />}
...
function AccountModal({ token, user, userChanged, focusFeedback = false, close, save, onError, notify, logout }: { token: string; user: CurrentUser; userChanged: (user: CurrentUser) => void; focusFeedback?: boolean; close: () => void; save: (profile: Partial<CurrentUser>) => void; onError: (e: unknown) => void; notify: (s: string) => void; logout: () => void }) {
