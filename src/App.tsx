...
function Dashboard({ token, user, setUser, logout, onError, notify }: { token: string; user: CurrentUser; setUser: (u: CurrentUser) => void; logout: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      {busy ? <FullPageLoader embedded /> : view.kind === 'home' ? <Home user={user} wishlists={wishlists} shared={shared} openWishlist={(w) => select({ kind: 'wishlist', wishlist: w })} openShared={(s) => select({ kind: 'shared', share: s })} newWishlist={() => setCreateOpen(true)} openShare={() => setShareOpen(true)} /> : view.kind === 'wishlist' ? <WishlistDetail token={token} wishlist={view.wishlist} onError={onError} notify={notify} /> : <SharedDetail token={token} accountId={user.id} share={view.share} onError={onError} onRemove={() => void removeShare(view.share)} onAdd={() => addShareToAccount(view.share)} />}
    </main>
...
function SharedDetail({ token, accountId, share, onError, onRemove, onAdd }: { token: string; accountId: string; share: SharedWishlist; onError: (e: unknown) => void; onRemove: () => void; onAdd: () => Promise<void> }) {
  ...
  return <div className="page detail-page shared-detail">
    ...
    <section className="save-shared-panel">
      ...
      <div className="save-shared-actions">
        ...
        {share.accountShareID && <button className="secondary danger-text" onClick={() => { if (window.confirm(`Remove “${share.title}” from Shared With Me? You’ll need the original link to add it again.`)) onRemove() }}><Trash2 /> Remove from account</button>}
      </div>
    </section>
    {loading ? <FullPageLoader embedded /> : rows.length ? <div className="items-grid">{rows.map((row) => <SharedItemCard key={row.item.id} row={row} chooseQuantity={(quantity) => quantity === 0 ? void update(row.item.id, { purchasedQuantity: 0 }) : setIdentity({ itemId: row.item.id, purchasedQuantity: quantity })} addNote={() => setNoteItem(row.item.id)} />)}</div> : <EmptyState icon={<Gift />} title="There’s nothing here yet" text="Check back after the list owner adds a wish." />}
    ...
    {noteItem && <NoteModal close={() => setNoteItem(null)} save={(note, displayName, shareName) => { const itemId = noteItem; setNoteItem(null); void update(itemId, { note, displayName, shareName }) }} />}
  </div>
}

function SharedItemCard({ row, chooseQuantity, addNote }: { row: SharedItemRow; chooseQuantity: (quantity: number) => void; addNote: () => void }) {
  ...
  return <article className={`item-card shared-item ${claimed >= requested ? 'purchased' : ''}`}><div className="item-icon">{claimed >= requested ? <PackageCheck /> : <Gift />}</div><div className="item-copy"><div className="item-title-row"><h3>{row.item.title}</h3>{row.item.price != null && <strong>{currency(row.item.price)}</strong>}</div><small>{claimed} of {requested} claimed</small>{row.item.ownerNote && <p>{row.item.ownerNote}</p>}{row.item.url && <a href={safeUrl(row.item.url)} target="_blank" rel="noreferrer">View item <ExternalLink /></a>}<div className="item-actions"><label className="quantity-choice"><span>You’re buying</span><select value={mine} onChange={(event) => chooseQuantity(Number(event.target.value))}>{Array.from({ length: maximumForMe + 1 }, (_, quantity) => <option key={quantity} value={quantity}>{quantity}</option>)}</select></label><button className="text-button" onClick={addNote}>Add a note</button></div>{row.notes.length > 0 && <div className="notes"><span>Notes</span>{row.notes.map((note, i) => <p key={`${note.updatedAt}-${i}`}><strong>{note.authorDisplayName || 'Anonymous'} · </strong>{note.note}</p>)}</div>}</div></article>
}
...
function NoteModal({ close, save }: { close: () => void; save: (note: string, displayName: string | undefined, shareName: boolean) => void }) { const [note, setNote] = useState(''); const [shareName, setShareName] = useState(false); const [name, setName] = useState(localStorage.getItem('hushful.displayName') || ''); return <Modal close={close}><ModalHeader eyebrow="For fellow gifters" title="Add a note" close={close} /><form className="stack-form" onSubmit={(e) => { e.preventDefault(); if (shareName) localStorage.setItem('hushful.displayName', name.trim()); save(note.trim(), shareName ? name.trim() : undefined, shareName) }}><Field label="Note"><textarea autoFocus rows={4} value={note} onChange={(e) => setNote(e.target.value)} placeholder="I can pick this up, or want to split it?" /></Field><label className="checkbox"><input type="checkbox" checked={shareName} onChange={(e) => setShareName(e.target.checked)} /><span><strong>Share my name on this note</strong><small>{shareName ? 'Other recipients will see your name beside this note.' : 'This note will appear as Anonymous.'}</small></span></label>{shareName && <Field label="Your name"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Display name" autoComplete="name" /></Field>}<div className="modal-actions"><button type="button" className="secondary" onClick={close}>Cancel</button><button className="primary" disabled={!note.trim() || (shareName && !name.trim())}>Add note</button></div></form></Modal> }
