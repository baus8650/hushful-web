...
function FriendsModal({ token, close, onError, notify, openShare }: { token: string; close: () => void; onError: (e: unknown) => void; notify: (s: string) => void; openShare: (share: AccountSharedWishlist) => void }) {
  ...
  if (profile) return <FriendProfileModal token={token} person={profile} close={() => setProfile(null)} openShare={openShare} onError={onError} />
  return <Modal close={close} size="modal-wide"><ModalHeader eyebrow="Your private circle" title="Friends & groups" close={close} /><div className="social-stack">
    ...
    <section><h3>Friends</h3>{friends.length ? friends.map((friend) => <SocialRow key={friend.id} person={friend.user} action={<div className="social-actions"><button className="secondary" onClick={() => setProfile(friend.user)}>View lists</button><button className="text-button danger-text" onClick={async () => { try { await api.removeFriendship(token, friend.id); await load() } catch (e) { onError(e) } }}>Remove</button></div>} />) : <p className="hint">Search for a username to start your private circle.</p>}</section>
...
function FriendProfileModal({ token, person, close, openShare, onError }: { token: string; person: SocialUser; close: () => void; openShare: (share: AccountSharedWishlist) => void; onError: (e: unknown) => void }) {
