...
function WishlistDetail({ token, wishlist, allWishlists, pinned, togglePin, onRenamed, onError, notify }: { token: string; wishlist: Wishlist; allWishlists: Wishlist[]; pinned: boolean; togglePin: () => void; onRenamed: (wishlist: Wishlist) => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <div className="page detail-page">
    <header className="page-heading"><div><p className="eyebrow">My wishlist</p><h1>{wishlist.title}</h1><p>{items.length} {items.length === 1 ? 'wish' : 'wishes'} tucked away · {wishlist.visibility === 'public' ? 'Public' : 'Private'}</p></div><div className="heading-actions"><button className="secondary" onClick={() => setRenaming(true)}><Pencil /> Rename</button><button className="secondary" onClick={togglePin}><Pin /> {pinned ? 'Unpin' : 'Pin'}</button><button className="secondary" onClick={() => setSharing(true)}><Share2 /> Sharing & privacy</button><button className="primary" onClick={() => setAddOpen(true)}><Plus /> Add item</button></div></header>
    ...
    {addOpen && <AddItemModal token={token} wishlist={wishlist} allWishlists={allWishlists} close={() => setAddOpen(false)} save={async (item) => { try { const created = await api.createItem(token, wishlist.id, item); setItems((all) => [created, ...all]); setAddOpen(false); notify('Wish added') } catch (e) { onError(e) } }} />}
    {editingItem && <AddItemModal token={token} wishlist={wishlist} allWishlists={allWishlists} initial={editingItem} close={() => setEditingItem(null)} save={async (changes) => { try { const updated = await api.updateItem(token, wishlist.id, editingItem.id, changes); setItems((all) => all.map((item) => item.id === updated.id ? updated : item)); setEditingItem(null); notify('Wish updated') } catch (e) { onError(e) } }} />}
...
function AddItemModal({ token, wishlist, allWishlists, initial, close, save }: { token: string; wishlist: Wishlist; allWishlists: Wishlist[]; initial?: WishlistItem; close: () => void; save: (item: { title: string; url?: string; price?: number; ownerNote?: string; quantity: number; linkedWishlistIDs: string[] }) => void }) {
