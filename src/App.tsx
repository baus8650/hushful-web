...
function Dashboard({ token, user, setUser, logout, onError, notify }: { token: string; user: CurrentUser; setUser: (u: CurrentUser) => void; logout: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  const tutorialKey = `hushful.tutorial.v2.seen.${user.id}`
...
function TutorialModal({ close }: { close: () => void }) { const [page, setPage] = useState(0); const steps = [{ title: 'Everything has a place', text: 'Use the sidebar for your wishlists, pinned shortcuts, friends, Find people, Activity, and your complete shared-list library.' }, { title: 'Share your way', text: 'Create a wishlist, add wishes, then share with selected friends or groups, make it public, or create a guest link for someone without an account.' }, { title: 'Find people and become friends', text: 'Find people searches discoverable usernames as you type. Open a profile to send a request; the other person must approve it from Activity.' }, { title: 'Pin what matters', text: 'Pin favorite owned or shared lists, friends, and groups for quick access. Pinning creates a shortcut—it never changes who can see anything.' }, { title: 'Privacy and activity', text: 'Activity holds requests and shared-list updates. Account & privacy controls your username, discoverability, friend requests, and profile picture.' }]; const step = steps[page]; return <Modal close={close} size="modal-wide"><div className="tutorial"><Logo compact /><TutorialPreview page={page} /><p className="eyebrow">Step {page + 1} of {steps.length}</p><h2>{step.title}</h2><p>{step.text}</p><div className="tutorial-dots">{steps.map((_, index) => <span key={index} className={index === page ? 'active' : ''} />)}</div><div className="modal-actions spread"><button className="text-button" onClick={close}>{page === steps.length - 1 ? 'Close' : 'Skip'}</button><button className="primary" onClick={() => page === steps.length - 1 ? close() : setPage(page + 1)}>{page === steps.length - 1 ? 'Start using Hushful' : 'Next'} <ChevronRight /></button></div></div></Modal> }
function TutorialPreview({ page }: { page: number }) { const titles = ['Hushful', 'Birthday Wishes', 'Find people', 'Pinned', 'Activity']; const controls = page === 0 ? <><TutorialCallout icon={<Users />} label="People" /><TutorialCallout icon={<Bell />} label="Activity" /></> : page === 1 ? <><TutorialCallout icon={<Share2 />} label="Share" /><TutorialCallout icon={<Plus />} label="Add item" /></> : page === 2 ? <TutorialCallout icon={<User />} label="Search" /> : page === 3 ? <TutorialCallout icon={<Pin />} label="Pinned" /> : <TutorialCallout icon={<Settings />} label="Privacy" />; return <div className="tutorial-preview"><div className="tutorial-preview-nav"><strong>{titles[page]}</strong><div>{controls}</div></div><div className="tutorial-preview-body">{page === 0 ? <><TutorialMiniRow title="Birthday Wishes" detail="Your wishlist" /><TutorialMiniRow title="Penny’s List" detail="Shared with me" /></> : page === 1 ? <><TutorialMiniRow title="Cozy blanket" detail="Private · Shared with Family" /><small>Choose friends, groups, public access, or a guest link.</small></> : page === 2 ? <><TutorialMiniRow title="Penny Lou" detail="@penny · View profile" /><div className="tutorial-detail-actions"><button>Send friend request</button></div></> : page === 3 ? <><TutorialMiniRow title="Penny’s Birthday" detail="Pinned list" /><TutorialMiniRow title="Family" detail="Pinned group" /></> : <><TutorialMiniRow title="Friend request" detail="Accept or decline" /><small>Control discoverability and requests in Account & privacy.</small></>}</div></div> }
...
type ItemFormValue = { title: string; url?: string; price?: number; ownerNote?: string; quantity: number; linkedWishlistIDs: string[] }
...
function AddItemModal({ token, wishlist, allWishlists, initial, close, save }: { token: string; wishlist: Wishlist; allWishlists: Wishlist[]; initial?: WishlistItem; close: () => void; save: (item: ItemFormValue, image: { file?: File; remove: boolean }) => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try { await save({ title: title.trim(), url: url.trim() || undefined, price: price ? Number(price.replace(',', '.')) : undefined, ownerNote: note.trim() || undefined, quantity, linkedWishlistIDs: [...linked] }, { file: imageFile, remove: removeImage }) }
  ...
  return <Modal close={close} size="modal-wide"><ModalHeader eyebrow={editing ? 'A thoughtful adjustment' : 'One more lovely thing'} title={editing ? 'Edit this wish' : 'Add a wish'} close={close} /><form className="stack-form" onSubmit={submit}>
    ...
    <div className="field-row"><Field label="Link (optional)"><input type="url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://…" /></Field><Field label="Price (optional)"><div className="money-input"><span>$</span><input inputMode="decimal" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0.00" /></div></Field></div>
    <Field label="Quantity"><input type="number" min="1" max="999" value={quantity} onChange={(e) => setQuantity(Math.max(1, Number(e.target.value) || 1))} /></Field><Field label="A note (optional)"><textarea rows={3} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Size, color, or why you love it…" /></Field>
