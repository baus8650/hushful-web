...
import { Check, ChevronRight, CircleHelp, Copy, ExternalLink, Gift, Link2, LoaderCircle, LogOut, Menu, PackageCheck, Pencil, Plus, RefreshCw, Send, Settings, Share2, Sparkles, Trash2, User, Users, X } from 'lucide-react'
import { api, ApiError } from './api'
import { authStorage, shareStorage } from './storage'
import type { CurrentUser, FriendGroup, Friendship, SharedItemRow, SharedWishlist, SocialUser, Wishlist, WishlistAudience, WishlistItem } from './types'
...
function Dashboard({ token, user, setUser, logout, onError, notify }: { token: string; user: CurrentUser; setUser: (u: CurrentUser) => void; logout: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <div className="app-shell">
    ...
    {friendsOpen && <FriendsModal token={token} close={() => setFriendsOpen(false)} onError={onError} notify={notify} />}
    {accountOpen && <AccountModal user={user} close={() => setAccountOpen(false)} save={async (profile) => { try { setUser(await api.updateProfile(token, profile)); setAccountOpen(false); notify('Privacy settings updated') } catch (e) { onError(e) } }} logout={logout} />}
...
function FriendsModal({ token, close, onError, notify }: { token: string; close: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <Modal close={close} size="modal-wide"><ModalHeader eyebrow="Your private circle" title="Friends & groups" close={close} /><div className="social-stack">
    ...
    {results.map((person) => <SocialRow key={person.id} person={person} action={<button className="secondary" disabled={friendIDs.has(person.id) || outgoingIDs.has(person.id)} onClick={async () => { try { await api.requestFriend(token, person.id); notify('Friend request sent'); await load() } catch (e) { onError(e) } }}>{friendIDs.has(person.id) ? 'Friends' : outgoingIDs.has(person.id) ? 'Requested' : 'Add friend'}</button>} />)}
    ...
    <section><h3>Friends</h3>{friends.length ? friends.map((friend) => <SocialRow key={friend.id} person={friend.user} action={<button className="text-button danger-text" onClick={async () => { try { await api.removeFriendship(token, friend.id); await load() } catch (e) { onError(e) } }}>Remove</button>} />) : <p className="hint">Search for a username to start your private circle.</p>}</section>
...
function SocialRow({ person, action }: { person: SocialUser; action: ReactNode }) { return <div className="social-row"><Avatar name={person.displayName || person.username} /><span><strong>{person.displayName || `@${person.username}`}</strong><small>@{person.username}</small></span>{action}</div> }
...
function SocialShareModal({ token, wishlist, close, notify, onError }: { token: string; wishlist: Wishlist; close: () => void; notify: (s: string) => void; onError: (e: unknown) => void }) {
  const [friends, setFriends] = useState<Friendship[]>([]), [groups, setGroups] = useState<FriendGroup[]>([]), [audience, setAudience] = useState<WishlistAudience>({ userIDs: [], groupIDs: [] }), [loading, setLoading] = useState(true), [saving, setSaving] = useState(false), [shareUrl, setShareUrl] = useState('')
  useEffect(() => { Promise.all([api.friends(token), api.friendGroups(token), api.wishlistAudience(token, wishlist.id)]).then(([f, g, a]) => { setFriends(f); setGroups(g); setAudience(a) }).catch(onError).finally(() => setLoading(false)) }, [token, wishlist.id, onError])
  ...
  async function save() { setSaving(true); try { await api.updateWishlistAudience(token, wishlist.id, audience); notify('Sharing updated'); close() } catch (e) { onError(e) } finally { setSaving(false) } }
  ...
  return <Modal close={close} size="modal-wide"><ModalHeader eyebrow="Share without a link" title={`Share “${wishlist.title}”`} close={close} />{loading ? <FullPageLoader embedded /> : <div className="social-stack"><p className="muted">Only the friends and private groups you select can open this wishlist.</p>
    ...
    <section><h3>Friends</h3>{friends.length ? friends.map((friend) => <label className="audience-choice" key={friend.user.id}><input type="checkbox" checked={audience.userIDs.includes(friend.user.id)} onChange={() => toggle('userIDs', friend.user.id)} /><Avatar name={friend.user.displayName || friend.user.username} /><span>{friend.user.displayName || `@${friend.user.username}`}<small>@{friend.user.username}</small></span></label>) : <p className="hint">Add friends first, or use a guest link below.</p>}</section>
...
function AccountModal({ user, close, save, logout }: { user: CurrentUser; close: () => void; save: (profile: Partial<CurrentUser>) => void; logout: () => void }) { const [name, setName] = useState(user.displayName || ''), [username, setUsername] = useState(user.username || ''), [discoverable, setDiscoverable] = useState(user.isDiscoverable), [policy, setPolicy] = useState(user.friendRequestPolicy); return <Modal close={close}><ModalHeader eyebrow="Your Hushful account" title="Account & privacy" close={close} /><form className="stack-form" onSubmit={(e) => { e.preventDefault(); save({ displayName: name.trim(), username: username.trim() || undefined, isDiscoverable: discoverable, friendRequestPolicy: policy }) }}><Field label="Display name"><input value={name} onChange={(e) => setName(e.target.value)} /></Field><Field label="Username"><input value={username} onChange={(e) => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} placeholder="your_username" /></Field><label className="checkbox"><input type="checkbox" checked={discoverable} onChange={(e) => setDiscoverable(e.target.checked)} /><span><strong>Let people find my username</strong><small>Your email is never shown in search.</small></span></label><Field label="Friend requests"><select value={policy} onChange={(e) => setPolicy(e.target.value as CurrentUser['friendRequestPolicy'])}><option value="everyone">Anyone who finds me</option><option value="nobody">Nobody</option></select></Field><Field label="Email"><input value={user.email} disabled /></Field><div className="modal-actions spread"><button type="button" className="text-button danger-text" onClick={logout}><LogOut /> Log out</button><button className="primary" disabled={!name.trim() || (discoverable && username.length < 3)}>Save changes</button></div></form></Modal> }
