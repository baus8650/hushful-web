...
function AccountModal({ token, user, userChanged, onSharedListsPreferenceChanged, focusFeedback = false, close, onError, notify, logout }: { token: string; user: CurrentUser; userChanged: (user: CurrentUser) => void; onSharedListsPreferenceChanged?: () => void; focusFeedback?: boolean; close: () => void; onError: (e: unknown) => void; notify: (s: string) => void; logout: () => void }) {
  ...
  async function grantProAccess(account: Awaited<ReturnType<typeof api.adminAccounts>>[number]) { const reason = window.prompt('Why should Pro access be granted for this account? Review Purchase feedback evidence first; an unverified claim is not proof.', 'Verified store purchase; manual recovery grant approved')?.trim(); if (!reason) return; setProGrantBusy(account.id); try { const grant = await api.grantProAccess(token, account.id, reason); setProGrants((current) => [...(current || []).filter((item) => item.userID !== account.id || !item.active), grant]); setAccounts((current) => current ? current.map((item) => item.id === account.id ? { ...item, isPro: true } : item) : current); notify('Pro access granted') } catch (e) { onError(e) } finally { setProGrantBusy(null) } }
  ...
  return <Modal close={close}><ModalHeader eyebrow="Your Hushful account" title="Account & privacy" close={close} /><form className="stack-form" onSubmit={(e) => e.preventDefault()}>
    ...
    {feedback && <section className="feedback-admin"><div><strong>User feedback</strong><small>{feedback.length} response{feedback.length === 1 ? '' : 's'}</small></div>{feedback.length === 0 ? <p className="hint">No feedback submitted yet.</p> : <div className="feedback-responses">{feedback.map((entry) => <article key={entry.id}><div><span>{entry.category}</span><small>{entry.platform.toUpperCase()} · {entry.createdAt ? new Date(entry.createdAt).toLocaleString() : 'Just now'}</small></div><p>{entry.message}</p>{entry.category === 'purchase' && <div className={`purchase-review ${purchaseEvidenceIsVerified(entry.purchaseEvidence) ? 'verified' : 'unverified'}`}><strong>{purchaseEvidenceLabel(entry.purchaseEvidence)}</strong><small>{entry.purchaseEvidenceDetails || 'No store verification was available. Do not grant Pro from this claim alone.'}</small>{entry.purchaseProvider && <small>Store: {entry.purchaseProvider === 'google_play' ? 'Google Play' : 'Apple'}{entry.purchaseAt ? ` · ${new Date(entry.purchaseAt).toLocaleString()}` : ''}{entry.purchaseOrderID ? ` · Order ${entry.purchaseOrderID}` : ''}</small>}</div>}<small>{entry.category === 'purchase' ? `Submitted by ${entry.userEmail || 'unknown account email'}` : `${entry.userDisplayName ? `${entry.userDisplayName} · ` : ''}${entry.userEmail || 'Anonymous'}`}</small></article>)}</div>}</section>}
