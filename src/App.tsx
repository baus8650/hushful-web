...
function Dashboard({ token, user, setUser, logout, onError, notify }: { token: string; user: CurrentUser; setUser: (u: CurrentUser) => void; logout: () => void; onError: (e: unknown) => void; notify: (s: string) => void }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-panel">
      ...
      {busy ? <FullPageLoader embedded /> : view.kind === 'home' ? <Home user={user} wishlists={wishlists} shared={shared} openWishlist={(w) => select({ kind: 'wishlist', wishlist: w })} openShared={(s) => select({ kind: 'shared', share: s })} newWishlist={() => setCreateOpen(true)} openShare={() => setShareOpen(true)} /> : view.kind === 'wishlist' ? <WishlistDetail token={token} wishlist={view.wishlist} onError={onError} notify={notify} /> : <SharedDetail token={token} accountId={user.id} share={view.share} onError={onError} onRemove={() => void removeShare(view.share)} />}
    </main>
...
function SharedDetail({ token, accountId, share, onError, onRemove }: { token: string; accountId: string; share: SharedWishlist; onError: (e: unknown) => void; onRemove: () => void }) {
