  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
      ...
      <nav>
        ...
        <button className="nav-home" onClick={() => { setOccasionsOpen(true); setMobileNav(false) }}><CalendarDays /> Occasions</button>
    ...
    {occasionsOpen && <OccasionsModal token={token} close={() => setOccasionsOpen(false)} onError={onError} notify={notify} createWishlist={createWishlist} />}
...
function AddItemModal({ token, wishlist, allWishlists, initial, close, save }: { token: string; wishlist: Wishlist; allWishlists: Wishlist[]; initial?: WishlistItem; close: () => void; save: (item: ItemFormValue, image: { file?: File; remove: boolean }) => Promise<void> }) {
  ...
  return <Modal close={close} size="modal-wide"><ModalHeader eyebrow={editing ? 'A thoughtful adjustment' : 'One more lovely thing'} title={editing ? 'Edit this wish' : 'Add a wish'} close={close} /><form className="stack-form" onSubmit={submit}>
    {!editing && <fieldset className="public-choice"><legend>What would you like to add?</legend><label className="checkbox"><input type="radio" checked={!cashFund} onChange={() => setCashFund(false)} /><span><strong>Wish</strong><small>A product or gift idea</small></span></label><label className="checkbox"><input type="radio" checked={cashFund} disabled={!wishlist.proAccess} onChange={() => setCashFund(true)} /><span><strong>Cash Fund</strong><small>{wishlist.proAccess ? 'An external contribution link' : 'Available with Hushful Pro in the app'}</small></span></label></fieldset>}
