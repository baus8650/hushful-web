...
function SharedDetail({ token, accountId, share, onError, onRemove, onAdd }: { token: string; accountId: string; share: SharedWishlist; onError: (e: unknown) => void; onRemove: () => void; onAdd: () => Promise<void> }) {
  ...
  useEffect(() => { void load() }, [load])
  async function update(itemId: string, body: { purchased?: boolean; note?: string; displayName?: string; shareName?: boolean }) { if (!viewerToken && !share.accountShareID) return; try { const row = share.accountShareID ? await api.updateAccountSharedItem(token, share.accountShareID, itemId, body) : await api.updateSharedItem(share.shareToken, itemId, viewerToken!, body); setRows((all) => all.map((r) => r.item.id === itemId ? row : r)) } catch (e) { onError(e); void load() } }
  return <div className="page detail-page shared-detail">
    <header className="page-heading"><div><p className="eyebrow">Shared by {share.sharedByName || 'Someone'}</p><h1>{share.title}</h1><p>Claims stay hidden from the person who made this list.</p></div><div className="heading-actions"><button className={share.accountShareID ? 'secondary' : 'primary'} disabled={saving || Boolean(share.accountShareID) || !share.shareToken} onClick={async () => { setSaving(true); try { await onAdd() } catch (e) { onError(e) } finally { setSaving(false) } }}>{saving ? <LoaderCircle className="spin" /> : share.accountShareID ? <Check /> : <Plus />}{share.accountShareID ? 'Saved to Shared With Me' : 'Add to Shared With Me'}</button><button className="secondary" onClick={() => void load()}><RefreshCw /> Refresh</button><button className="icon-button danger" aria-label="Remove saved list" onClick={onRemove}><Trash2 /></button></div></header>
